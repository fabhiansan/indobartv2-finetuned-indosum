...
def generate_summaries(
    ...
    # Define a safe decoding function that handles out-of-range token IDs
    def safe_decode(token_ids, skip_special_tokens=True):
        try:
            # Try the normal decoding first
            return tokenizer.decode(token_ids, skip_special_tokens=skip_special_tokens)
        except IndexError as e:
            logger.warning(f"Caught IndexError during decoding: {e}")
    ...
    raw_dataset = eval_dataset.map(
        lambda x: {
            input_column: safe_decode(x["input_ids"], skip_special_tokens=True),
            target_column: safe_decode(x["labels"], skip_special_tokens=True),
        }
    )
    ...
    for i in range(0, len(raw_dataset), batch_size):
        ...
        # Decode generated summaries using our safe decoding method
        decoded_summaries = []
        for ids in generated_ids:
            decoded_summaries.append(safe_decode(ids, skip_special_tokens=True))
